...
public:
    ...
    bool send(const lspc::Serializable& obj) { return send(obj.type(), obj.serialize()); }
    ...
    bool unregisterCallback(uint8_t type)
    {
        ...
        if (handler_it == type_handlers.end()) {
            return false; // callback not registered already registered - this ensures that we can not overwrite an
                          // existing registered callback
        }
