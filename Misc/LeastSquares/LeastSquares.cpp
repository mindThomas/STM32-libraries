...
float LeastSquares1D::GetEstimate()
{
	...
	if (fabs(a_squared) < 0.01) return 0;
...
{
	A_init_[0] = 0; A_init_[1] = 0; A_init_[2] = 0; A_init_[3] = 0;
	y_init_[0] = 0; y_init_[1] = 0;
}
...
bool LeastSquares2D::GetEstimate(float x_est[2])
{
	...
	// Check for singularity (not sufficient samples added)
	if (fabs(AtA[0]*AtA[3] - AtA[1]*AtA[2]) < 0.01) return false;
