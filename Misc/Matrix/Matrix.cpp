...
{
	if (row >= rows_ || cols_ == 0) { tmp = 0; return tmp; }
...
{
	if (row >= rows_ || cols_ == 0) { return tmp; }
...
{
	if (row >= rows_ || col >= cols_) { tmp = 0; return tmp; }
...
{
	if (row >= rows_ || col >= cols_) { return tmp; }
...
void Matrix::transpose(Matrix& out) const
{
	// out = M^T
	if (rows_ == out.cols_ ||
		cols_ == out.rows_) return; // error - sizes do not match
...
arm_status Matrix_Multiply(const float * A, uint8_t A_rows, uint8_t A_cols, const float * B, uint8_t B_rows, uint8_t B_cols, float * out, uint8_t out_rows, uint8_t out_cols)
{
	// See ARM-CMSIS DSP library for matrix operations: https://www.keil.com/pack/doc/CMSIS/DSP/html/group__groupMatrix.html
	arm_matrix_instance_f32 A_; arm_mat_init_f32(&A_, A_rows, A_cols, (float32_t *)A);
	arm_matrix_instance_f32 B_; arm_mat_init_f32(&B_, B_rows, B_cols, (float32_t *)B);
	arm_matrix_instance_f32 out_; arm_mat_init_f32(&out_, out_rows, out_cols, out);
