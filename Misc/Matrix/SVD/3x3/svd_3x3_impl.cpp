...
// Return Type  : void
//
void b_svd_3x3(const float A[9], float U[9], float s[3], float V[9])
{
    int       i;
    float     b_A[9];
    float     b_s[3];
    ...
    for (q = 0; q < 2; q++) {
        ...
        for (k = q + 1; k + 1 < 4; k++) {
            i = q + 3 * k;
            if (apply_transform) {
                xaxpy_3x3(3 - q, -(xdotc_3x3(3 - q, b_A, qq + 1, b_A, i + 1) / b_A[q + 3 * q]), qq + 1, b_A, i + 1);
            }
        ...
        if (q + 1 <= 1) {
            ...
            if (nrm == 0.0F) {
                ...
                for (k = 1; k + 1 < 4; k++) {
                    b_xaxpy_3x3(2, e[k], b_A, 3 * k + 2, work, 2);
                }

                for (k = 1; k + 1 < 4; k++) {
                    c_xaxpy_3x3(2, -e[k] / e[1], work, 2, b_A, 3 * k + 2);
                }
            }
    ...
    for (q = 1; q >= 0; q--) {
        ...
        if (b_s[q] != 0.0F) {
            for (k = q + 1; k + 1 < 4; k++) {
                i = (q + 3 * k) + 1;
                xaxpy_3x3(3 - q, -(xdotc_3x3(3 - q, U, qq + 1, U, i) / U[qq]), qq + 1, U, i);
            }
    ...
    for (q = 2; q >= 0; q--) {
        if ((q + 1 <= 1) && (e[0] != 0.0F)) {
            for (k = 2; k < 4; k++) {
                i = 3 * (k - 1) + 2;
                xaxpy_3x3(2, -(xdotc_3x3(2, V, 2, V, i) / V[1]), 2, V, i);
            }
        }
    ...
    while ((m + 2 > 0) && (!(iter >= 75))) {
        ...
        do {
            ...
            if (k + 1 == 0) {
                ...
                if ((nrm <= 1.1920929E-7F * ((float)fabs((double)b_s[k]) + (float)fabs((double)b_s[k + 1]))) ||
        ...
        switch (i) {
            case 1:
                ...
                e[m] = 0.0F;
                for (k = m; k + 1 >= q + 1; k--) {
                    xrotg_3x3(&b_s[k], &f, &nrm, &r);
                    if (k + 1 > q + 1) {
                        f = -r * e[0];
                        e[0] *= nrm;
                    }

                    xrot_3x3(V, 1 + 3 * k, 1 + 3 * (m + 1), nrm, r);
                }
                break;
            ...
            case 2:
                ...
                for (k = q; k < m + 2; k++) {
                    ...
                    f = -r * e[k];
                    e[k] *= nrm;
                    xrot_3x3(U, 1 + 3 * k, 1 + 3 * (q - 1), nrm, r);
                }
                break;
            ...
            case 3:
                ...
                for (k = q + 1; k <= m + 1; k++) {
                    ...
                    rt       = r * b_s[k];
                    b_s[k] *= nrm;
                    xrot_3x3(V, 1 + 3 * (k - 1), 1 + 3 * k, nrm, r);
                    b_s[k - 1] = f;
                    xrotg_3x3(&b_s[k - 1], &rt, &nrm, &r);
                    f      = nrm * e[k - 1] + r * b_s[k];
                    b_s[k] = -r * e[k - 1] + nrm * b_s[k];
                    rt     = r * e[k];
                    e[k] *= nrm;
                    xrot_3x3(U, 1 + 3 * (k - 1), 1 + 3 * k, nrm, r);
                }
            ...
            default:
                ...
                while ((q + 1 < 3) && (b_s[q] < b_s[i])) {
                    ...
                    b_s[q] = b_s[i];
                    b_s[i] = rt;
                    xswap_3x3(V, 1 + 3 * q, 1 + 3 * (q + 1));
                    xswap_3x3(U, 1 + 3 * q, 1 + 3 * (q + 1));
                    q = i;
                    i++;
                }
