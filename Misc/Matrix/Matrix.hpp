...
public:
	static Matrix zeros(uint8_t num_elements) { // zero-vector constructor
		 Matrix out(num_elements);
		 out.zero();
		 return out;
	}

	static Matrix zeros(uint8_t num_rows, uint8_t num_cols) { // matrix constructor
	...
	{
		// out = A * B
		if (out.rows_ != A.rows_ ||
			out.cols_ != B.cols_ ||
			A.cols_ != B.rows_) return; // error - sizes do not match
	...
	static inline void add(const Matrix& A, const Matrix& B, Matrix& out)
	{
		...
		if (out.rows_ == A.rows_ ||
			out.cols_ == A.cols_ ||
			A.rows_ != B.rows_ ||
			A.cols_ != B.cols_) return; // error - sizes do not match
	...
	{
		// out = M^T
		if (M.rows_ == out.rows_ ||
			M.cols_ == out.cols_) return; // error - sizes do not match
	...
	static inline void inverse(const Matrix& M, Matrix& out)
	{
		...
		if (M.rows_ != M.cols_) return;
		if (out.rows_ != M.rows_ ||
			out.cols_ != M.cols_) return;
	...
	static inline void svd(const Matrix& M, Matrix& U, Matrix& S, Matrix& V)
	{
		...
		if (M.rows_ != M.cols_) return;
		if (U.rows_ != M.rows_ ||
			U.cols_ != M.cols_) return;
		if (S.rows_ != M.rows_ ||
			S.cols_ != M.cols_) return;
		if (V.rows_ != M.rows_ ||
			V.cols_ != M.cols_) return;
	...
	inline void negate()
	{
		arm_negate_f32(*this, *this, this->size_);
	}
