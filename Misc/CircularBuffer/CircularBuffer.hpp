...
public:
    ...
    {
        if (!_buffer)
            return; // error, buffer memory not available
        if (!_allowOverrun && AvailablePackets() >= _bufferSize)
        ...
        if (!_allowOverrun && _bufferWriteIdx == _bufferReadIdx) {
            if (_overrunDetected) { // overrun detected in previous write, so now we need to discard an old sample by
    ...
    void PushFromInterrupt(T packet)
    {
        if (!_buffer)
            return; // error, buffer memory not available
        if (AvailablePackets() >= _bufferSize)
            return; // error, buffer is full
    ...
    {
        T* popBuffer = 0;
        if (numberOfPacketsToPop > AvailablePackets())
    ...
    bool Available() { return (_bufferWriteIdx != _bufferReadIdx); }
