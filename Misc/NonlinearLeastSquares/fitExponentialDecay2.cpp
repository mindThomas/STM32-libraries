...
static float __anon_fcn(const cell_wrap_1 error_tunableEnvironment[2], float a,
  float b, float c);
  ...
  float b, float c)
{
  ...
  for (i1 = 0; i1 < firstBlockLength; i1++) {
    b_varargout_1->data[i1] = error_tunableEnvironment[0].f1->data[i1] - (a + b *
      b_varargout_1->data[i1]);
  }
  ...
  for (k = 1; k <= (int)varargout_1_idx_0; k++) {
    ...
    y->data[c_y[0] * (k - 1)] = rt_powf_snf(b_varargout_1->data[d_varargout_1[0]
      * (k - 1)], 2.0F);
  }
...
// Return Type  : void
//
void fitExponentialDecay2(emxArray_real32_T *x, const emxArray_real32_T *y,
  float *a, float *b, float *c)
{
  ...
  for (i0 = 0; i0 < 2; i0++) {
    emxCopyStruct_cell_wrap_1(&error_tunableEnvironment[i0],
  ...
  do {
    ...
    if ((b_a->size[1] == 1) || (Jr_->size[0] == 1)) {
      ...
      for (i0 = 0; i0 < 3; i0++) {
        for (itmp = 0; itmp < 3; itmp++) {
          b_y[i0 + 3 * itmp] = 0.0F;
          for (p2 = 0; p2 < p1; p2++) {
            b_y[i0 + 3 * itmp] += Jr_->data[i0 + Jr_->size[1] * p2] * b_a->
              data[p2 + b_a->size[1] * itmp];
          }
        }
      }
    } else {
      for (p2 = 0; p2 < 3; p2++) {
        for (p1 = 0; p1 < 3; p1++) {
          ...
          for (p3 = 0; p3 < b_a->size[1]; p3++) {
            temp += b_a->data[p3 + b_a->size[1] * p2] * Jr_->data[p1 + Jr_->
              size[1] * p3];
          }
    ...
    b_y[p1 % 3 * 3 + p1 / 3] = ((1.0F - b_x[1] * absx21) - b_x[2] * temp) / b_x
      [0];
    ...
    p1 = y->size[0];
    for (i0 = 0; i0 < p1; i0++) {
      varargin_1->data[i0] = y->data[i0] - (coeffs[0] + coeffs[1] *
        varargin_1->data[i0]);
    }
    ...
    if ((b_a->size[1] == 1) || (varargin_1->size[0] == 1)) {
      ...
      for (i0 = 0; i0 < 3; i0++) {
        ...
        for (itmp = 0; itmp < p3; itmp++) {
          temp = b_varargin_1[i0] + varargin_1->data[itmp] * b_a->data[itmp +
            b_a->size[1] * i0];
      ...
      for (p2 = 0; p2 < 3; p2++) {
        ...
        for (p3 = 0; p3 < b_a->size[1]; p3++) {
          ...
          temp += b_a->data[p3 + b_a->size[1] * p2] * varargin_1->
            data[d_varargin_1[0] * p3];
        }
