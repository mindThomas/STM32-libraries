...
/* OBS! This file only serves as a template file and is supposed to be copied into the project root include folder which
 * will effectively take over the definitions in this template */

/* Define hardware interrupt priorities   (HAL_NVIC_SetPriority)
 * Lower value means higher priority
 * Interrupt priority has to be between 0 and 15
 * Remember to respect the "configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY" in FreeRTOSConfig.h, hence no hardware
 * interrupt calling FreeRTOS functions must have a priority value lower than
 * configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY
 */
#define HIGH_RES_TIMER_TICK_PRIORITY                                                                                   \
    0 // This timer is also used to generate ticks for any ST HAL library (eg. HAL_Delay)
