...
SPI::SPI(port_t port)
    : SPI(port, SPI_DEFAULT_FREQUENCY)
{}
...
void SPI::ConfigurePeripheral()
{
    ...
    if (!_hRes->configured) { // only configure periphiral once
        ...
        _hRes->handle.Init.NSS =
          SPI_NSS_SOFT; // general chip-select - we have decided not to support hardware chip-select (NSS pin)
        ...
        uint32_t SPI_Clock = HAL_RCCEx_GetPeriphCLKFreq(
          RCC_PERIPHCLK_SPI123); // assuming all SPI periphirals to be configured with the same clock frequency!!!
...
void SPI::TransactionWrite32(uint32_t value)
{
    ...
    uint8_t tx[4] = {(uint8_t)((value >> 24) & 0xFF), (uint8_t)((value >> 16) & 0xFF), (uint8_t)((value >> 8) & 0xFF),
                     (uint8_t)((value)&0xFF)};
    uint8_t rx[4] = {0};
