...
void SyncedPWMADC::InitADCs()
{
    ...
    hADC_Config.Init.ClockPrescaler =
      ADC_CLOCK_SYNC_PCLK_DIV4; // Take ADC PLL-based clock (168 MHz) and divide with this Prescaler
...
void SyncedPWMADC::ADC_ConfigureCurrentSenseSampling()
{
    ...
    if (_SamplingEnabled || LL_ADC_REG_IsConversionOngoing(hADC1.Instance) ||
        LL_ADC_REG_IsConversionOngoing(hADC2.Instance)) {
        ShouldRestart = true;
    }
    if (ShouldRestart) {
        StopSampling();
...
void SyncedPWMADC::InitDMAs()
{
    ...
    if (!SAMPLE_IN_EVERY_PWM_CYCLE) {
        ...
        hDMA_ADC1.Init.Mode = DMA_CIRCULAR; // we want to sample at every PWM period, so it should just run continuously
    ...
    if (!SAMPLE_IN_EVERY_PWM_CYCLE) {
        ...
        hDMA_ADC2.Init.Mode = DMA_CIRCULAR; // we want to sample at every PWM period, so it should just run continuously
                                            // in a circular fashion
        timerSettingsNext.samplingInterval            = 1;
        timerSettingsNext.samplingAveragingNumSamples = 1;
    }
...
{
    if (!_ADCEnabled)
        return; // sampling already stopped
    _ADCEnabled = false;
...
void SyncedPWMADC::StartSampling()
{
    ...
    __HAL_TIM_CLEAR_IT(&hTimer, TIM_IT_UPDATE); // important to clear the interrupt before enabling since the update
                                                // flag might be set without the interrupt having been enabled, and thus
                                                // the interrupt would end up firing right after enabling it
