...
void SyncedPWMADC::InitTimer(uint32_t frequency)
{
    ...
    __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_1); // Enable preload to ensure that duty cycles will only be
                                                         // changed at the beginning of each PWM period
    if (HAL_TIM_PWM_ConfigChannel(&hTimer, &sConfigOC, TIM_CHANNEL_3) != HAL_OK)
        ERROR("Could not configure PWM channel");
    __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_3); // Enable preload to ensure that duty cycles will only be
                                                         // changed at the beginning of each PWM period
    ...
    if (DEBUG_MODE_ENABLED) {
        if (HAL_TIM_PWM_ConfigChannel(&hTimer, &sConfigOC, TIM_CHANNEL_2) != HAL_OK)
            ERROR("Could not configure PWM channel");
        __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_2); // Enable preload to ensure that duty cycles will only be
                                                             // changed at the beginning of each PWM period
        if (HAL_TIM_PWM_ConfigChannel(&hTimer, &sConfigOC, TIM_CHANNEL_4) != HAL_OK)
            ERROR("Could not configure PWM channel");
        __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_4); // Enable preload to ensure that duty cycles will only be
                                                             // changed at the beginning of each PWM period
    }
    ...
    if (HAL_TIM_PWM_ConfigChannel(&hTimer, &sConfigOC, TIM_CHANNEL_5) != HAL_OK)
        ERROR("Could not configure PWM channel");
    __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_5); // Enable preload to ensure that duty cycles will only be
                                                         // changed at the beginning of each PWM period
    if (HAL_TIM_PWM_ConfigChannel(&hTimer, &sConfigOC, TIM_CHANNEL_6) != HAL_OK)
        ERROR("Could not configure PWM channel");
    __HAL_TIM_ENABLE_OCxPRELOAD(&hTimer, TIM_CHANNEL_6); // Enable preload to ensure that duty cycles will only be
                                                         // changed at the beginning of each PWM period
...
{
    if (numSamples > 0 && numSamples <= MAX_SAMPLING_NUM_SAMPLES &&
        numSamples * timerSettingsNext.Triggers.numEnabledTriggers * Channels.ADCnumChannels[0] <=
          ADC_DMA_HALFWORD_MAX_BUFFER_SIZE &&
        numSamples * timerSettingsNext.Triggers.numEnabledTriggers * Channels.ADCnumChannels[1] <=
          ADC_DMA_HALFWORD_MAX_BUFFER_SIZE &&
        (numSamples < timerSettingsNext.samplingInterval - 1 ||
         numSamples == 1)) // Sampling interval needs to be at least 2 more sample than the averaging interval (2 sample
                           // dead-time to allow sufficient time to compute/carry out the DMA sampling finished
                           // interrupt - it is especially the averaging that takes time, so if many samples are
                           // collected, averaging will consume a big amount of time)
    {
        xSemaphoreTake(_timerSettingsMutex, (TickType_t)portMAX_DELAY); // lock settings for change
        timerSettingsNext.samplingAveragingNumSamples =
          numSamples; // capture samples from one PWM period after every interval (hence every PWM period)
        timerSettingsNext.InvalidateSamples =
          1; // these timer setting changes invalidates the current sample since it might affect the sample ordering
        timerSettingsNext.Changed = true;
        xSemaphoreGive(_timerSettingsMutex); // unlock settings
...
void TIM1_UP_TIM16_IRQHandler(void)
{
    ...
    if (__HAL_TIM_GET_FLAG(&SyncedPWMADC::globalObject->hTimer, TIM_FLAG_UPDATE) != RESET &&
        ...
        TIM16->SR = ~(uint32_t)(TIM_IT_UPDATE | TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_CC3 | TIM_IT_CC4 | TIM_IT_COM |
