...
void CANBus::ConfigurePeripheral()
{
    ...
    if (HAL_FDCAN_ConfigGlobalFilter(&_hRes->handle, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_ACCEPT_IN_RX_FIFO0,
    ...
    if (HAL_FDCAN_ConfigInterruptLines(&_hRes->handle, FDCAN_IT_RX_FIFO0_NEW_MESSAGE, FDCAN_INTERRUPT_LINE0) !=
        HAL_OK) {
        ERROR("Failed configuring CAN RX/TX notification");
    }

    /* Activate Tx Completed notification */
    if (HAL_FDCAN_ActivateNotification(&_hRes->handle, FDCAN_IT_TX_COMPLETE,
