...
UART::UART(port_t port, uint32_t baud, uint32_t bufferLength, bool DMA_enabled) : BaudRate(baud), DMA_Enabled(DMA_enabled), _port(port), _rxByteAvailable(false), _rxAvailableOnIdle(false), _bufferLength(bufferLength), _bufferWriteIdx(0), _bufferReadIdx(0),
		...
		_transmitFinished(0), _RXdataAvailable(0), _RXcallback(0), _RXcallbackParameter(0)
#else
UART::UART(port_t port, uint32_t baud, uint32_t bufferLength) : BaudRate(baud), _port(port), _rxByteAvailable(false), _rxAvailableOnIdle(false), _bufferLength(bufferLength), _bufferWriteIdx(0), _bufferReadIdx(0),
		_callbackChunkLength(0), _txPointer(0), _txRemainingBytes(0),
		_transmitFinished(true), _RXcallback(0), _RXcallbackParameter(0)
#endif
{
...
UART::UART(port_t port, uint32_t baud, bool DMA_enabled) : UART(port, baud, 0, DMA_enabled)
{
}
...
void UART::ConfigurePeripheral()
{
	...
	if (!DMA_Enabled) {
		...
		/* Enable the UART Parity Error interupt and Data Register (RX) Not Empty interrupt (since FIFO mode is disabled) */
...
void UART::RegisterRXcallback(void (*callback)UART_CALLBACK_PARAMS, void * parameter, uint32_t chunkLength)
{
	...
	switch (_port) {
		case PORT_UART2:
			xTaskCreate(UART::CallbackThread, (char *)"UART2 callback", 512, (void*) this, UART_RECEIVER_PRIORITY, &_callbackTaskHandle);
			break;
		default:
			break;
	}
...
// Slight modification to the "HAL_UART_Transmit_DMA" method from "stm32g4xx_hal_uart.c" to point to a different DMA Transmit Completed handler
void UART::StartDMATransfer(uint8_t *pData, uint16_t Size)
{
	...
	if (_handle.hdmatx != NULL)
	{
		...
		if (HAL_DMA_Start_IT(_handle.hdmatx, (uint32_t)_handle.pTxBuffPtr, (uint32_t)&_handle.Instance->TDR, Size) != HAL_OK)
		{
...
bool UART::Available()
{
	if (!_bufferLength) return _rxByteAvailable; // error, buffer not enabled
...
{
	if (DMA_Enabled) return; // Buffer push can not be used in DMA mode
	if (!_bufferLength) {
		rxByte = byte;
