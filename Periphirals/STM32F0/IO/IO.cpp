...
{
#ifdef USE_FREERTOS
    if (!_GPIO || !_isInput || (!_InterruptSemaphore && !_InterruptCallback))
        return; // no interrupt configured
    _InterruptSemaphore = 0;
#else
    if (!_GPIO || !_isInput || !_InterruptCallback)
        return; // no interrupt configured
#endif
    _InterruptCallback       = 0;
    _InterruptCallbackParams = 0;
...
void IO::ConfigureInterrupt(interrupt_trigger_t trigger)
{
    ...
    uint32_t SYSCFG_EXTI_LINE =
      ((uint32_t)(4 * (pinIndex & 0b0011))) << 16 |
      ((pinIndex & 0b1100) >> 2); // according to SYSTEM_LL_EC_EXTI_LINE in stm32f0xx_ll_sysytem.h
