...
{
	if (!_hRes) return 0;
	return -( (uint16_t)LL_TIM_GetCounter(_hRes->instance) + _hRes->offsetValue); // invert sign due to direction of motors and assignment of encoder channels A+B
}
...
void TIM2_IRQHandler(void)
{
	if (Encoder::resTIMER2)
		...
		TIM2->SR = ~(uint32_t)(TIM_SR_UIF | TIM_SR_CC1IF | TIM_SR_CC2IF | TIM_SR_CC3IF | TIM_SR_CC4IF | TIM_SR_COMIF | TIM_SR_TIF | TIM_SR_BIF); // clear all interrupts
}
...
void TIM3_IRQHandler(void)
{
	if (Encoder::resTIMER3)
		...
		TIM3->SR = ~(uint32_t)(TIM_SR_UIF | TIM_SR_CC1IF | TIM_SR_CC2IF | TIM_SR_CC3IF | TIM_SR_CC4IF | TIM_SR_COMIF | TIM_SR_TIF | TIM_SR_BIF); // clear all interrupts
}
