...
Servo::Servo(timer_t timer, pwm_channel_t channel,  float min, float max, float min_ms, float max_ms, uint16_t range_resolution_steps) :
	PWM(timer, channel, 50, (uint16_t)roundf(20.0f / ((max_ms - min_ms) / range_resolution_steps))), // 50 Hz = 20 ms with '_ms_resolution' resolution
	_min_ms(min_ms),
	_min_value(min),
	_max_ms(max_ms),
	_max_value(max)
