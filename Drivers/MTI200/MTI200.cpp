...
bool MTI200::Configure(uint32_t SampleRate)
{
    ...
    while (tries-- > 0)
        if (configureMotionTracker(SampleRate))
            break;
    while (tries-- > 0)
        if (sendCommand(XMID_GotoMeasurement))
            break;
        ...
        if (msgPtr) {
            ...
            if (xQueueSend(mti200->_responseQueue, (void*)&msgPtr, (TickType_t)0) != pdPASS) {
                deallocateMessageData(
                  message->data); // failed putting message into queue - deallocate message data memory
            }
        } else {
...
bool MTI200::configureMotionTracker(uint32_t SampleRate)
{
    ...
    if (deviceId) {
        ...
        if ((968 * SampleRate) > _uart->BaudRate)
            Debug::print("Note: MTI sample rate might be too high to ensure consistent sampling without data loss\n");
        OutputConfiguration conf[] = {
          ...
          //{XDI_DeltaQ, SampleRate}, // no need to include the DeltaQ, since this is just generated from the RateOfTurn
          //and corresponds to the movement over a period of 1/(400 Hz)
        ...
        return setOutputConfiguration(conf, sizeof(conf) / sizeof(OutputConfiguration));
    }
...
void MTI200::printMessageData(XbusMessage const* message)
{
    ...
    if (XbusMessage_getDataItem(ori, XDI_Quaternion, message)) {
        Debug::printf(" Orientation: (% .3f, % .3f, % .3f, % .3f)", ori[0], ori[1], ori[2], ori[3]);
    }
    float acc[3];
...
void MTI200::GetEstimates(Estimates_t& estimates)
{
    ...
    Quaternion_GetDQ_FromBody(estimates.q, LastMeasurement.Gyroscope,
                              estimates.dq); // convert angular velocity into q_dot
