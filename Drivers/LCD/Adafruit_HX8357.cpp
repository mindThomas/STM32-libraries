...
*/
Adafruit_HX8357::Adafruit_HX8357(SPI* spi, IO* dcPin, IO* rstPin, uint8_t type)
    : Adafruit_SPITFT(HX8357_TFTWIDTH, HX8357_TFTHEIGHT, spi, dcPin, rstPin)
    , displayType(type)
{}
...
static const uint8_t initb[] =
  {
    ...
    0              // END OF COMMAND LIST
                     ...
                     initd[] = {
                       HX8357_SWRESET,
                       0x80 + 10 / 5, // Soft reset, then delay 10 ms
                       HX8357D_SETC,
                       3,
                       0xFF,
                       0x83,
                       0x57,
                       0xFF,
                       0x80 + 300 / 5, // No command, just delay 300 ms
                       HX8357_SETRGB,
                       4,
                       0x80,
                       0x00,
...
void Adafruit_HX8357::begin(uint32_t type)
{
    ...
    while ((cmd = pgm_read_byte(addr++)) > 0) { // '0' command ends list
        if (cmd != 0xFF)
            writeCommand(cmd); // '255' is ignored
...
/*!
    ...
    @return  None (void).
*/
void Adafruit_HX8357::setAddrWindow(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h)
{
